...
if __name__ == '__main__':
    ...
    # for idx_sub in range(sub, 0, -1):
        ...
        # @数据集及产生器
        # dataset_tra = train_BCIDataset(num_data, data_tra_list[0], win_data, label_tra_list[0], label_tra_list[0],
        #                                down_sample, channel)
        ...
        for data_tra, label_tra, start_time_tra in zip(data_tra_list, label_tra_list, start_time_tra_list):
            ...
            gen_tr = DataLoader(dataset_tra, shuffle=True, batch_size=bth_size, num_workers=1, pin_memory=True, drop_last=True)
            gen_tra.append(gen_tr)
        ...
        for epoch in range(num_epo_tra):
            ...
            # @网络训练及Batch循环
            net.train()
            for exp_nu in range(exp - 2):
                for iter_tra, bth_tra in enumerate(gen_tra[exp_nu]):  # iteration是批次，batch是每批次的数据
                    ...
                    loss_tra_epo_log.append(loss_tra_bth)  # 将当前批次的损失值添加到损失日志列表中。
            ...
            mid_loss_tra_epo = sum(loss_tra_epo_log) / epo_size / exp_nu  # 计算当前周期的平均损失。
    ...
    # @训练-验证-测试后的文件保存
    path_loss_tra_glo_log = './logs/SubAll_loss_tra_glo_log.mat'
    path_loss_val_glo_log = './logs/SubAll_loss_val_glo_log.mat'
    path_loss_tes_glo_log = './logs/SubAll_loss_tes_glo_log.mat'
    path_acc_val_glo_log = './logs/SubAll_acc_val_glo_log.mat'
    path_acc_tes_glo_log = './logs/SubAll_acc_tes_glo_log.mat'
    path_pdt_tes_glo_log = './logs/SubAll_pdt_tes_glo_log.mat'
    path_tgt_tes_glo_log = './logs/SubAll_tgt_tes_glo_log.mat'
